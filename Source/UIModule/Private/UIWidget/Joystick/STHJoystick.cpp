...
void STHJoystick::Construct(const FArguments& InArgs)
{
	...
	FSlateApplication::Get().GetPlatformApplication()->OnDisplayMetricsChanged().AddSP(this, &STHJoystick::HandleDisplayMetricsChanged);
}

void STHJoystick::HandleDisplayMetricsChanged(const FDisplayMetrics& NewDisplayMetric)
{
...
void STHJoystick::SetGlobalParameters(float InActiveOpacity, float InInactiveOpacity, float InTimeUntilDeactive, float InTimeUntilReset, float InActivationDelay, bool InbPreventReCenter, float InStartupDelay)
{
...
bool STHJoystick::ShouldDisplayTouchInterface()
{
...
void STHJoystick::RecoverFocusToJoystick()
{
	FSlateApplication::Get().SetAllUserFocus(AsShared());
}

int32 STHJoystick::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	if (bVisible)
	{
		...
		for (int32 ControlIndex = 0; ControlIndex < Controls.Num(); ControlIndex++)
		{
			const FJoystickControlInfo& Control = Controls[ControlIndex];
			
			// BGI
			const FSlateBrush* JoystickBGIBrush = JoystickBGI.Get();
			if (bShowVisualBGI && JoystickBGIBrush && JoystickBGIBrush->DrawAs != ESlateBrushDrawType::NoDrawType)
			{
				FSlateDrawElement::MakeBox(
					...
					AllottedGeometry.ToPaintGeometry(
						...
						Control.CorrectedVisualSize),			// 实时计算CorrectedVisualSize 但是这个因数可以提取
					JoystickBGIBrush,
			...
			if (ControlIndex == 0)
			{
				const FSlateBrush* JoystickThumbImageBrush = JoystickThumbImage.Get();
				if (bShowThumbImg && JoystickThumbImageBrush && JoystickThumbImageBrush->DrawAs != ESlateBrushDrawType::NoDrawType)
				{
					FSlateDrawElement::MakeBox(
						OutDrawElements,
						RetLayerId++,
			...
			else
			{
				// Thumb
				const FSlateBrush* JoystickThumbImageBrush = JoystickThumbImage.Get();
				if (JoystickThumbImageBrush && JoystickThumbImageBrush->DrawAs != ESlateBrushDrawType::NoDrawType)
				{
					FSlateDrawElement::MakeBox(
						OutDrawElements,
						RetLayerId++,
...
FVector2D STHJoystick::ComputeDesiredSize(float) const
{
	return FVector2D(100, 100);
}

bool STHJoystick::SupportsKeyboardFocus() const
{
	return false;
}

void STHJoystick::ClearAllPlayerOperate()
{
	if (Controls.Num() == 0)
	{
		UE_LOG(LogTemp, Display, TEXT("STHJoystick::ClearAllPlayerOperate, Controls[0].VisualCenter == 0"));
		return;
	}

	HandleTouch(EJoystickHandleTouchType::Touch_End, 0, Controls[0].VisualCenter, FVector2D(1920.f, 1080.f));

	// release and center the joystick
	Controls[0].ThumbPosition = FVector2D(0, 0);
	Controls[0].CapturedPointerIndex = -1;

	// send one more joystick update for the centering
	Controls[0].bSendOneMoreEvent = true;

	// Pass event as unhandled if time is too short
	if (Controls[0].bNeedUpdatedCenter)
	{
		Controls[0].bNeedUpdatedCenter = false;
	}

	//FSlateApplication::Get().ClearAllUserFocus();
}

FReply STHJoystick::OnTouchStarted(const FGeometry& MyGeometry, const FPointerEvent& Event)
{
	FVector2D LocalCoord = MyGeometry.AbsoluteToLocal(Event.GetScreenSpacePosition());

	for (int32 ControlIndex = 0; ControlIndex < Controls.Num(); ControlIndex++)
	{
		FJoystickControlInfo& Control = Controls[ControlIndex];
		...
		if (Control.CapturedPointerIndex == -1)
		{
			if (PositionIsInside(Control.CorrectedCenter, LocalCoord, Control.CorrectedInteractionSize))
			{
				...
				if (ActivationDelay == 0.f)
				{
					...
					if (HandleTouch(EJoystickHandleTouchType::Touch_Start, ControlIndex, LocalCoord, MyGeometry.GetLocalSize())) // Never fail!
...
FReply STHJoystick::OnTouchMoved(const FGeometry& MyGeometry, const FPointerEvent& Event)
{
	FVector2D LocalCoord = MyGeometry.AbsoluteToLocal(Event.GetScreenSpacePosition());
	...
	for (int32 ControlIndex = 0; ControlIndex < Controls.Num(); ControlIndex++)
	{
		...
		if (Control.CapturedPointerIndex == Event.GetPointerIndex())
		{
			...
			else if (HandleTouch(EJoystickHandleTouchType::Touch_Move, ControlIndex, LocalCoord, MyGeometry.GetLocalSize()))
			{
...
FReply STHJoystick::OnTouchEnded(const FGeometry& MyGeometry, const FPointerEvent& Event)
{
	FVector2D LocalCoord = MyGeometry.AbsoluteToLocal(Event.GetScreenSpacePosition());
	for (int32 ControlIndex = 0; ControlIndex < Controls.Num(); ControlIndex++)
	{
		...
		if (Control.CapturedPointerIndex == Event.GetPointerIndex())
		{
			// left joystick
			if (ControlIndex == 0)
				HandleTouch(EJoystickHandleTouchType::Touch_End, ControlIndex, LocalCoord, MyGeometry.GetLocalSize());
...
bool STHJoystick::HandleTouch(EJoystickHandleTouchType HandleTouchType, int32 ControlIndex, const FVector2D& LocalCoord, const FVector2D& ScreenSize)
{
	...
	if (Offset == FVector2D(0, 0))
	{
		...
		ThumbOrienInfo.ThumbOffset = Offset;

		// ThumbOrientaion更新
		ThumbOrienInfo.Angle = 0.f;
		ThumbOrienInfo.CosAngle = 0.f;
		ThumbOrienInfo.SinAngle = 0.f;
		ThumbOrienInfo.XTerm = 0.f;
		ThumbOrienInfo.YTerm = 0.f;
		ThumbOrienInfo.DistanceToEdge = 0.f;
		ThumbOrienInfo.LocalCoord = LocalCoord;
	}
	else
	{
		...
		// only clamp ; 保证ThumbPos位于圆(摇杆BG)内
		...
		// ThumbOrientaion更新
		ThumbOrienInfo.ThumbOffset = Offset;
		ThumbOrienInfo.Angle = Angle;
		ThumbOrienInfo.CosAngle = CosAngle;
		ThumbOrienInfo.SinAngle = SinAngle;
		ThumbOrienInfo.XTerm = XTerm;
		ThumbOrienInfo.YTerm = YTerm;
		ThumbOrienInfo.DistanceToEdge = DistanceToEdge;
		ThumbOrienInfo.LocalCoord = LocalCoord;
	}

	if (UMG)
		UMG->HandleJoystickTouch(HandleTouchType, ControlIndex, LocalCoord, ScreenSize);
...
// Tick
void STHJoystick::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	// UE_LOG(LogTemp, Log, TEXT("STHJoystick::Tick( bPlayerPawnAutoRun=%d )"), bPlayerPawnAutoRun);
	...
	for (int32 ControlIndex = 0; ControlIndex < Controls.Num(); ControlIndex++)
	{
		...
		if (Control.bNeedUpdatedCenter)
		{
			...
			if (Control.ElapsedTime > ActivationDelay)
			{
				...
				HandleTouch(EJoystickHandleTouchType::Touch_Tick, ControlIndex, Control.NextCenter, AllottedGeometry.GetLocalSize());
			}
		}
		...
		if (!Control.bHasBeenPositioned)
		{
			...
			if (UMG)
				UMG->OnJoystickInitPropertyUpdateDelegate();
		}

		if (Control.CapturedPointerIndex >= 0 || Control.bSendOneMoreEvent)
		{	
			bool tbIsTouchEndedTick = Control.bSendOneMoreEvent;
			...
			//const FGamepadKeyNames::Type XAxis = (Control.MainInputKey.IsValid() ? Control.MainInputKey.GetFName() : (ControlIndex == 0 ? FName(TEXT("Gamepad_LeftX")) : FName(TEXT("Gamepad_RightX"))));
			//const FGamepadKeyNames::Type YAxis = (Control.AltInputKey.IsValid() ? Control.AltInputKey.GetFName() : (ControlIndex == 0 ? FName(TEXT("Gamepad_LeftY")) : FName(TEXT("Gamepad_RightY"))));


			////FSlateApplication::Get().SetAllUserFocusToGameViewport();		// 此函数将导致UI失焦

			////JoystickTickInfo.ThumbScaledOffset = ThumbScaledOffset;

			//// 进位至>=1; 且Distance2Edge * 相关角度, 保证X和Y值的比例正确，来达到控制角色根据摇杆方向转向.
			//float Angle = FMath::Atan2(ThumbScaledOffset.Y, ThumbScaledOffset.X);
			//float CosAngle = FMath::Cos(Angle * 1.f);
			//float SinAngle = FMath::Sin(Angle * 1.f);
			//float XTerm = CosAngle / (Control.CorrectedVisualSize.X * 0.5f);
			//float YTerm = SinAngle / (Control.CorrectedVisualSize.Y * 0.5f);
			//float DistanceToEdge = FMath::InvSqrt(XTerm * XTerm + YTerm * YTerm);

			//float tXAxisPoint = DistanceToEdge * CosAngle;
			//float tYAxisPoint = DistanceToEdge * SinAngle;

			//if (FVector2D::ZeroVector == NormalizedOffset)
			//{
			//	tXAxisPoint = tYAxisPoint = 0.f;
			//}

			//FSlateApplication::Get().OnControllerAnalog(XAxis, 0, tXAxisPoint);
			//FSlateApplication::Get().OnControllerAnalog(YAxis, 0, -tYAxisPoint);

			//JoystickTickInfo.NormalizedOffset = NormalizedOffset;
			if (UMG)
				UMG->ThumbOffsetTickDelegate(NormalizedOffset.X, NormalizedOffset.Y, tbIsTouchEndedTick);
		}
...
void STHJoystick::SetJoystickVisibility(const bool bInVisible, const bool bInFade)
{
...
void STHJoystick::AlignBoxIntoScreen(FVector2D& Position, const FVector2D& Size, const FVector2D& ScreenSize)
{
...
// Set Image ----------------------------------------------------------------------------
void STHJoystick::SetJoystickThumbImage(TAttribute<const FSlateBrush*> InImage)
{
	if (JoystickThumbImage.IsBound() || InImage.IsBound() || (JoystickThumbImage.Get() != InImage.Get()))
	{
		JoystickThumbImage = InImage;
		Invalidate(EInvalidateWidget::Layout);
	}
}
void STHJoystick::SetJoystickBGI(TAttribute<const FSlateBrush*> InImage)
{
	if (JoystickBGI.IsBound() || InImage.IsBound() || (JoystickBGI.Get() != InImage.Get()))
	{
		JoystickBGI = InImage;
		Invalidate(EInvalidateWidget::Layout);
	}
}
void STHJoystick::SetFastRunArrowImage(TAttribute<const FSlateBrush*> InImage)
{
	if (FastRunArrowImage.IsBound() || InImage.IsBound() || (FastRunArrowImage.Get() != InImage.Get()))
	{
		FastRunArrowImage = InImage;
		Invalidate(EInvalidateWidget::Layout);
	}
}
void STHJoystick::SetFastRunButtonImage(TAttribute<const FSlateBrush*> InImage)
{
	if (FastRunButtonImage.IsBound() || InImage.IsBound() || (FastRunButtonImage.Get() != InImage.Get()))
	{
		FastRunButtonImage = InImage;
		Invalidate(EInvalidateWidget::Layout);
	}
}
void STHJoystick::SetFastRunActiveImage(TAttribute<const FSlateBrush*> InImage)
{
	if (FastRunActiveImage.IsBound() || InImage.IsBound() || (FastRunActiveImage.Get() != InImage.Get()))
	{
		FastRunActiveImage = InImage;
		Invalidate(EInvalidateWidget::Layout);
	}
}
void STHJoystick::SetAutoRunThumbImage(TAttribute<const FSlateBrush*> InImage)
{
	if (AutoRunThumbImage.IsBound() || InImage.IsBound() || (AutoRunThumbImage.Get() != InImage.Get()))
	{
		AutoRunThumbImage = InImage;
		Invalidate(EInvalidateWidget::Layout);
	}
}

void STHJoystick::RefreshUI_RepositionOrResize()
{
	for (int32 ControlIndex = 0; ControlIndex < Controls.Num(); ControlIndex++)
	{
		Controls[ControlIndex].bHasBeenPositioned = false;
...
void STHJoystick::ShowTumbImg(bool InbShow)
{
	bShowThumbImg = InbShow;
}

void STHJoystick::ShowVisualBGI(bool InbShow)
{
	bShowVisualBGI = InbShow;
}
